  ...
  return (
    <article lang={lang} className='relative flex flex-col w-full h-full m-0 p-0'>
      <div id={id} className='w-full flex max-md:flex-row-reverse flex-nowrap gap-0'>
        <div className={`flex flex-col items-start justify-start md:flex-1 max-w-75 lg:bg-primary-700 p-0 md:pt-12 ${lang === "en" ? 'md:pr-2 lg:pr-6' : 'md:pl-2 lg:pl-6'}`}>
          <div className={`w-24 md:w-full flex-1 flex items-end justify-end bg-blue-500/25 p-2 md:p-3 max-md:rounded-b-full max-md:mx-px md:-mt-px ${lang === "en" ? 'md:rounded-r-full' : 'md:rounded-l-full'}`}>
            <img src={profileImage} alt="ProfileImage"
              className='w-full md:w-40 aspect-square object-cover overflow-hidden rounded-full border border-primary-500 shadow-[0_0_6px_-1px_var(--color-primary-600)]' />
          </div>
        </div>
